...
// Slow down rate of blur to make sure that we do not apply too much blur at a given time (up to 60 times per second)
final static int FRAMES_TO_SKIP_BETWEEN_MOUSE_DRAGS = 4;
...
void blur (float r) {
  ...
  ellipse(x, y, r*2, r*2);
  if (!mousePressed || frameCount % FRAMES_TO_SKIP_BETWEEN_MOUSE_DRAGS != 0) {
    return;
  }
